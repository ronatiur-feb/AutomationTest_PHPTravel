// Generated by Selenium IDE
import org.junit.Test;
import org.junit.Before;
import org.junit.After;
import static org.junit.Assert.*;
import static org.hamcrest.CoreMatchers.is;
import static org.hamcrest.core.IsNot.not;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.firefox.FirefoxDriver;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.remote.RemoteWebDriver;
import org.openqa.selenium.remote.DesiredCapabilities;
import org.openqa.selenium.Dimension;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.interactions.Actions;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.WebDriverWait;
import org.openqa.selenium.JavascriptExecutor;
import org.openqa.selenium.Alert;
import org.openqa.selenium.Keys;
import java.util.*;
import java.net.MalformedURLException;
import java.net.URL;
public class FlightTestTest {
  private WebDriver driver;
  private Map<String, Object> vars;
  JavascriptExecutor js;
  @Before
  public void setUp() {
    driver = new ChromeDriver();
    js = (JavascriptExecutor) driver;
    vars = new HashMap<String, Object>();
  }
  @After
  public void tearDown() {
    driver.quit();
  }
  @Test
  public void flightTest() {
    driver.get("https://www.phptravels.net/flights");
    driver.manage().window().setSize(new Dimension(1382, 754));
    {
      WebElement element = driver.findElement(By.cssSelector(".col-lg-4:nth-child(2) .deal__title"));
      Actions builder = new Actions(driver);
      builder.moveToElement(element).perform();
    }
    driver.findElement(By.cssSelector(".col-lg-4:nth-child(2) .deal__title")).click();
    {
      WebElement element = driver.findElement(By.tagName("body"));
      Actions builder = new Actions(driver);
      builder.moveToElement(element, 0, 0).perform();
    }
    {
      WebElement element = driver.findElement(By.linkText("Back to Home"));
      Actions builder = new Actions(driver);
      builder.moveToElement(element).perform();
    }
    driver.findElement(By.linkText("Back to Home")).click();
    {
      WebElement element = driver.findElement(By.tagName("body"));
      Actions builder = new Actions(driver);
      builder.moveToElement(element, 0, 0).perform();
    }
    driver.findElement(By.cssSelector(".nav-item:nth-child(2) .d-none")).click();
    driver.findElement(By.cssSelector(".col-md-4:nth-child(2) .form-check-label")).click();
    driver.findElement(By.id("round-trip")).click();
    driver.findElement(By.cssSelector(".col-md-4:nth-child(1) > .form-check > .form-check-label")).click();
    driver.findElement(By.id("one-way")).click();
    driver.findElement(By.id("one-way")).click();
    driver.findElement(By.linkText("1")).click();
    driver.findElement(By.id("flights-search")).click();
    driver.findElement(By.cssSelector(".col-md-4:nth-child(2) .form-check-label")).click();
    driver.findElement(By.cssSelector(".ladda-label")).click();
    driver.findElement(By.cssSelector(".intro_title")).click();
    driver.findElement(By.id("one-way")).click();
    driver.findElement(By.linkText("Back to Home")).click();
  }
}
